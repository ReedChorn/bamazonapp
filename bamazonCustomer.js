...
function promptUserPurchase() {
	...
	inq.prompt([
		...
		config.query(queryStr, {item_id: item}, function(err, data) {
			if (err) throw err;
			...
			if (data.length === 0) {
				...
				if (quantity <= productData.stock_quantity) {
					...
					config.query(updateQueryStr, function(err, data) {
						if (err) throw err;

						console.log('Your order has been placed! Your total is $' + productData.price * quantity);
						console.log('Thank you for shopping with us and have a great day!');
						console.log("\n---------------------------------------------------------------------\n");

						// End the database config
						config.end();
					})
				} else {
...
function displayInventory() {
	...
	// Make the db query
	config.query(queryStr, function(err, data) {
		if (err) throw err;
